...
## Output: data/processed/nlcd/nlcd_wisconsin_lc_2023.RData
...
wi_lc_2023 <- terra::as.data.frame(rast4e, xy = TRUE)
# Change column names
colnames(wi_lc_2023) <- c('x', 'y', 'ID')

# Color palette associatd with NLCD data
legend <- FedData::pal_nlcd()
legend <- legend |>
  # Take only land cover types in our dataset
  dplyr::filter(ID %in% wi_lc_2023$ID) |>
...
wi_lc_2023 <- dplyr::left_join(x = wi_lc_2023,
                               y = dplyr::select(legend, ID, Class),
                               by = 'ID')

# Save
save(wi_lc_2023,
     file = 'data/processed/nlcd/nlcd_wisconsin_lc_2023.RData')
...
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = 'figures/nlcd/wisconsin_all_cover_2023.png',
                height = 10, width = 10, units = 'cm')
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = '/Volumes/FileBackup/SDM_bigdata/nlcd_svgs/wisconsin_all_cover_2023.svg',
                height = 10, width = 10, units = 'cm')
...
# Save
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = 'figures/nlcd/wisconsin_red_cover_2023.png',
                height = 10, width = 10, units = 'cm')
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = '/Volumes/FileBackup/SDM_bigdata/nlcd_svgs/wisconsin_red_cover_2023.svg',
                height = 10, width = 10, units = 'cm')
