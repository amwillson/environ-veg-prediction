...
## 3. Plot Illinois cutout
## 4. Plot Illinois subset at 30 m resolution

## Input: /Volumes/FileBackup/SDM_bigdata/NLCD_PbV7RYKpbYgOXQRMcA2D/Annual_NLCD_LndCov_1985_CU_C1V0_PbV7RYKpbYgOXQRMcA2D.tiff
## NLCD raster for Illinois
## Downloaded from https://www.mrlc.gov/viewer/

## Output: data/processed/nlcd/nlcd_illinois_lc_1985.RData
...
il_lc_1985 <- terra::as.data.frame(rast5e, xy = TRUE)
# Change column names
colnames(il_lc_1985) <- c('x', 'y', 'ID')

# Color palette associatd with NLCD data
legend <- FedData::pal_nlcd()
legend <- legend |>
  # Take only land cover types in our dataset
  dplyr::filter(ID %in% il_lc_1985$ID) |>
...
il_lc_1985 <- dplyr::left_join(x = il_lc_1985,
                               y = dplyr::select(legend, ID, Class),
                               by = 'ID')

# Save
save(il_lc_1985,
     file = 'data/processed/nlcd/nlcd_illinois_lc_1985.RData')

#### 3. Plot Illinois cutout ####

# Plot Illinois with bounding box to show area of interest
ggplot2::ggplot() +
  ggplot2::geom_sf(data = illinois, color = NA, fill = 'grey85') +
  ggplot2::geom_rect(ggplot2::aes(xmin = 351898.018970391,
                                  xmax = 515598.789959032,
                                  ymin = 339134.739332346,
                                  ymax = 562302.686714322),
                     color = 'red', fill = 'red', alpha = 0.5) +
  ggplot2::geom_sf(data = illinois, color = 'black', fill = NA) +
  ggplot2::theme_void()

# Save
ggplot2::ggsave(plot = ggplot2::last_plot(),
                filename = 'figures/nlcd/il_subset.png',
...
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = 'figures/nlcd/illinois_all_cover_1985.png',
                height = 10, width = 10, units = 'cm')
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = '/Volumes/FileBackup/SDM_bigdata/nlcd_svgs/illinois_all_cover_1985.svg',
                height = 10, width = 10, units = 'cm')
...
# Save
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = 'figures/nlcd/illinois_red_cover_1985.png',
                height = 10, width = 10, units = 'cm')
ggplot2::ggsave(plot = ggplot2::last_plot() + ggplot2::theme(legend.position = 'none'),
                filename = '/Volumes/FileBackup/SDM_bigdata/nlcd_svgs/illinois_red_cover_1985.svg',
                height = 10, width = 10, units = 'cm')
