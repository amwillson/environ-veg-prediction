...
## Output: /Volumes/FileBackup/SDM_bigdata/out/gjam/H/abundance/allcovar.RData
...
# Run model
abund_gjam_H_allcovar <- gjam::gjam(formula = ~ clay + sand + silt + caco3 + awc + flood +
                                      pptsum + tmean + pptcv +
...
# Simple plots
gjam::gjamPlot(abund_gjam_H_allcovar)

# Save
# Change directory according to your file structure
save(abund_gjam_H_allcovar,
     file = '/Volumes/FileBackup/SDM_bigdata/out/gjam/H/abundance/allcovar.RData')
...
sens <- abund_gjam_H_allcovar$chains$fSensGibbs
...
# sensitivity figure
corrs <- cor(dplyr::select(xdata, -flood, -silt, -sand,
                           -pptcv, -caco3, -pptsum,
                           -awc))
...
# How about tmax, tmeansd, clay
corrs2 <- cor(dplyr::select(xdata, tmax, tmeansd, clay))
corrs2
...
bFacGibbs <- abund_gjam_H_allcovar$chains$bFacGibbs
...
ggplot2::ggsave(plot = ggplot2::last_plot(),
                filename = 'figures/gjam/H/abundance/fit/all_covar_covariates.png',
