...
## Output: out/gam/M/density/predicted_historical_gam1.RData
...
## Output: out/gam/M/density/predicted_historical_gam2.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample 
## grid cells using the alternate model with only climate covariates
## Used in 7.7.density_figures.R

## Output: out/gam/M/density/predicted_historical_gam3.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample 
## grid cells using the alternate model with only the reduced set of covariates
## Used in 7.7.density_figures.R

## Output: out/gam/M/density/predicted_historical_gam4.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample
...
## Output: out/gam/M/density/predicted_historical_gam1_4k.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample
## grid cells using the main model with climate and soil covariates
...
## Output: out/gam/M/density/predicted_historical_gam2_4k.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample
## grid cells using the alternate model with only climate covariates
## with lower maximum basis dimensionality
...
## Output: out/gam/M/density/predicted_historical_gam3_4k.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample
## grid cells using the alternate model with only the reduced set of covariates
## with lower maximum basis dimensionality
...
## Output: out/gam/M/density/predicted_historical_gam4_4k.RData
## Dataframe of observed vegetation, soil, and climate data and
## predicted historical total stem density from the out-of-sample
## grid cells using the alternate model with the soil and climate
## covariates as well as the latitude and the longitude of the grid cell
...
pred_historical_gam1 <- mvgam::predictions(model = density_gam_M_allcovar,
                                           newdata = pls_oos)
...
save(pred_historical_gam1,
     file = 'out/gam/M/density/predicted_historical_gam1.RData')

# Make prediction with model 2
pred_historical_gam2 <- mvgam::predictions(model = density_gam_M_climcovar,
                                           newdata = pls_oos)
...
save(pred_historical_gam2,
     file = 'out/gam/M/density/predicted_historical_gam2.RData')

# Make predictions with model 3
pred_historical_gam3 <- mvgam::predictions(model = density_gam_M_redcovar,
                                           newdata = pls_oos)
...
save(pred_historical_gam3,
     file = 'out/gam/M/density/predicted_historical_gam3.RData')

# Make predictions with model 4
pred_historical_gam4 <- mvgam::predictions(model = density_gam_M_xycovar,
                                           newdata = pls_oos)

# Extract predictions
pred <- dplyr::select(pred_historical_gam4, estimate:conf.high)
...
# Add to original dataframe
pred_historical_gam4 <- cbind(pred, pls_oos)

# Save
save(pred_historical_gam4,
     file = 'out/gam/M/density/predicted_historical_gam4.RData')

# Make predictions with model 1, lower basis dimension
pred_historical_gam1_4k <- mvgam::predictions(model = density_gam_M_allcovar_4k,
                                              newdata = pls_oos)
...
save(pred_historical_gam1_4k,
     file = 'out/gam/M/density/predicted_historical_gam1_4k.RData')

# Make predictions with model 2, lower basis dimension
pred_historical_gam2_4k <- mvgam::predictions(model = density_gam_M_climcovar_4k,
                                              newdata = pls_oos)
...
save(pred_historical_gam2_4k,
     file = 'out/gam/M/density/predicted_historical_gam2_4k.RData')

# Make predictions with model 3, lower basis dimension
pred_historical_gam3_4k <- mvgam::predictions(model = density_gam_M_redcovar_4k,
                                              newdata = pls_oos)
...
save(pred_historical_gam3_4k,
     file = 'out/gam/M/density/predicted_historical_gam3_4k.RData')

# Make predictions with model 4, lower basis dimension
pred_historical_gam4_4k <- mvgam::predictions(model = density_gam_M_xycovar_4k,
                                              newdata = pls_oos)

# Extract predictions
pred <- dplyr::select(pred_historical_gam4_4k, estimate:conf.high)
...
# Add to original dataframe
pred_historical_gam4_4k <- cbind(pred, pls_oos)

# Save
save(pred_historical_gam4_4k,
     file = 'out/gam/M/density/predicted_historical_gam4_4k.RData')
