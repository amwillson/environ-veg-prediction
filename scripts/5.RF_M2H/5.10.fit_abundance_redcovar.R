...
## based on importance and minimum depth analysis performed in step 5-8:
...
## Output: /Volumes/FileBackup/SDM_bigdata/out/rf/M/abundance/redcovar.RData
...
rf_data <- fia_in |>
  ...
  dplyr::select(Ash, Basswood, Beech,
                ...
                ppt_sum, tmean_sd, tmax) |> # climatic variables
...
## Use nodesize = 2, mtry = 2 because there is minimal
## change in error rate between mtry = 2 and 4

#### 3. Fit random forest ####

# Fit random forest
abund_rf_M_redcovar <- randomForestSRC::rfsrc(formula = Multivar(Ash, Basswood, Beech,
                                                                 Birch, Cherry, Dogwood,
                                                                 Elm, Fir, Hemlock,
                                                                 Hickory, Ironwood, Maple,
                                                                 Oak, Pine, Spruce,
...
save(abund_rf_M_redcovar,
     file = '/Volumes/FileBackup/SDM_bigdata/out/rf/M/abundance/redcovar.RData')
