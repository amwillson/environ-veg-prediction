...
# Put columns in the same order as with the PLS data
xydata_modern <- xydata_modern |>
  dplyr::ungroup() |>
...
```{r pred-rf1-modern}
# Make predictions
pred_modern_rf1 <- randomForestSRC::predict.rfsrc(object = density_rf_allcovar,
                                                  newdata = xydata_modern)

# Extract predictions
pred_out_rf1 <- pred_modern_rf1$predicted

# Bind predictions and out-of-sample data for analysis
pred_modern_rf1 <- cbind(xydata_modern, pred_out_rf1)
...
save(pred_modern_rf1,
     file = 'out/total_density/rf/predicted_modern_density_rf1.RData')
```

```{r pred-rf2-modern}
# Make predictions
pred_modern_rf2 <- randomForestSRC::predict.rfsrc(object = density_rf_climcovar,
                                                  newdata = xydata_modern)

# Extract predictions
pred_out_rf2 <- pred_modern_rf2$predicted

# Bind predictions and out-of-sample data for analysis
pred_modern_rf2 <- cbind(xydata_modern, pred_out_rf2)
...
save(pred_modern_rf2,
     file = 'out/total_density/rf/predicted_modern_density_rf2.RData')
```

```{r pred-rf3-modern}
# Make predictions
pred_modern_rf3 <- randomForestSRC::predict.rfsrc(object = density_rf_redcovar,
                                                  newdata = xydata_modern)

# Extract predictions
pred_out_rf3 <- pred_modern_rf3$predicted

# Bind predictions and out-of-sample data for analysis
pred_modern_rf3 <- cbind(xydata_modern, pred_out_rf3)
...
save(pred_modern_rf3,
     file = 'out/total_density/rf/predicted_modern_density_rf3.RData')
```

```{r pred-rf4-modern}
# Make predictions
pred_modern_rf4 <- randomForestSRC::predict.rfsrc(object = density_rf_xycovar,
                                                  newdata = xydata_modern)

# Extract predictions
pred_out_rf4 <- pred_modern_rf4$predicted

# Bind predictions and out-of-sample data for analysis
pred_modern_rf4 <- cbind(xydata_modern, pred_out_rf4)
...
save(pred_modern_rf4,
     file = 'out/total_density/rf/predicted_modern_density_rf4.RData')
```

Here are some preliminary checks on prediction accuracy.

```{r pred-accuracy-modern}
# Plot observation vs prediction for each random forest
pred_modern_rf1 |>
  ggplot2::ggplot() +
  ggplot2::geom_point(ggplot2::aes(x = total_density, y = pred_out_rf1)) +
...
# Correlation coefficients
r_rf1 <- cor(x = pred_modern_rf1$total_density,
             y = pred_modern_rf1$pred_out_rf1)
r_rf2 <- cor(x = pred_modern_rf2$total_density,
             y = pred_modern_rf2$pred_out_rf2)
r_rf3 <- cor(x = pred_modern_rf3$total_density,
             y = pred_modern_rf3$pred_out_rf3)
r_rf4 <- cor(x = pred_modern_rf4$total_density,
             y = pred_modern_rf4$pred_out_rf4)

corrs_modern <- c(r_rf1, r_rf2, r_rf3, r_rf4)

methods = c('Climate + soil', 'Climate only',
            'Top four', 'Climate + soil + coordinates')

corrs_modern <- as.data.frame(cbind(methods, corrs_modern))

tibble::tibble(corrs_modern)
```

To do a quick comparison, let's do paired bars of correlation coefficient with historical vs modern predictions.

```{r r-comp}
# Combine
corrs_both <- cbind(corrs, corrs_modern$corrs_modern)
# Colnames
colnames(corrs_both) <- c('Method', 'Historical', 'Modern')
...
corrs_both |>
  ...
  dplyr::mutate(r = as.numeric(r),
                Method = dplyr::if_else(Method == 'Climate + soil + coordinates',
                                        'Climate + soil +\ncoordinates',
                                        Method)) |>
  ggplot2::ggplot() +
  ggplot2::geom_bar(ggplot2::aes(x = Method, y = r, fill = Period),
                    stat = 'identity', 
...
# Alternative out-of-sample modern prediction

Instead of using all the grid cells, we could just limit the analysis to those grid cells that we withheld from the PLS-era data. This section repeats the entire prediction and comparison for the modern era above, but only using this subset of grid cells.

```{r filter-oos-modern}
# Take grid cells from PLS
pls_oos_cells <- pls_oos |>
  dplyr::select(x, y) |>
  dplyr::mutate(in_pls = TRUE)

# Add whether or not they were in PLS to modern OOS
xydata_modern_subset <- xydata_modern |>
  dplyr::left_join(y = pls_oos_cells,
                   by = c('x', 'y')) |>
  dplyr::filter(in_pls == TRUE) |>
  dplyr::select(-in_pls)
```

Now that we have only the grid cells in common, we can do our predictions.

```{r pred-rf1-modern2}
# Make predictions
pred_modern_rf1 <- randomForestSRC::predict.rfsrc(object = density_rf_allcovar,
                                                  newdata = xydata_modern_subset)

# Extract predictions
pred_out_rf1 <- pred_modern_rf1$predicted

# Bind predictions and out-of-sample data for analysis
pred_modern_rf1 <- cbind(xydata_modern_subset, pred_out_rf1)
...
# Save
save(pred_modern_rf1,
     file = 'out/total_density/rf/predicted_modern_density_sub_rf1.RData')
```
...
# Bind predictions and out-of-sample data for analysis
pred_modern_rf2 <- cbind(xydata_modern_subset, pred_out_rf2)
...
# Save
save(pred_modern_rf2,
     file = 'out/total_density/rf/predicted_modern_density_sub_rf2.Rdata')
```
...
# Bind predictions and out-of-sample data for analysis
pred_modern_rf3 <- cbind(xydata_modern_subset, pred_out_rf3)
...
# Save
save(pred_modern_rf3,
     file = 'out/total_density/rf/predicted_modern_density_sub_rf3.RData')
```
...
# Bind predictions and out-of-sample data for analysis
pred_modern_rf4 <- cbind(xydata_modern_subset, pred_out_rf4)
...
# Save
save(pred_modern_rf4,
     file = 'out/total_density/rf/predicted_modern_density_sub_rf4.RData')
```

```{r pred-accuracy-modern2}
# Plot observation vs prediction for each random forest
pred_modern_rf1 |>
  ggplot2::ggplot() +
  ggplot2::geom_point(ggplot2::aes(x = total_density, y = pred_out_rf1)) +
...
corrs_modern2 <- c(r_rf1, r_rf2, r_rf3, r_rf4)

methods = c('Climate + soil', 'Climate only',
            'Top four', 'Climate + soil + coordinates')

corrs_modern2 <- as.data.frame(cbind(methods, corrs_modern2))

tibble::tibble(corrs_modern2)
```

```{r r-comp2}
# Combine
corrs_both2 <- cbind(corrs, corrs_modern2$corrs_modern2)
# Colnames
colnames(corrs_both2) <- c('Method', 'Historical', 'Modern')
